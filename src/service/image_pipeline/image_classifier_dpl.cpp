...
#include <string>
#include <array>
#include "image_classifier_dpl.hpp"
#include "frame_client.cpp"
...
public:
    StaticActionTable() : table({{"pet",AT_PET_BREED},{"flower",AT_FLOWER_NAME},{"cow",AT_COW_RECOG}}) {}
...
static std::vector<std::string> parse_request(const std::string &key) {
    std::vector<std::string> request_components;
    const char* delim = "/";
    char buf[1024];
    std::strcpy(buf, key.c_str());
    char *comp = std::strtok(buf, delim);
    while (comp != nullptr) {
        request_components.push_back(std::string(comp));
        comp = std::strtok(NULL, delim);
    }
    return request_components; // RVO
}
...
class ClassifierFilter: public CriticalDataPathObserver<CascadeType> {
    virtual void operator() (const uint32_t sgidx,
        ...
        dbg_default_trace("Triggered critical data path");
        if constexpr (std::is_same<CascadeType,VolatileCascadeStoreWithStringKey>::value) {
            ...
            if (parse_request(key)[3].compare("lo") == 0 && members[hash % members.size()] == ctxt->get_service_client_ref().get_my_id()) {
                Action act;
                act.action_type = static_action_table.to_action(value.get_key_ref());
                // act.immediate_data is not used.
                // TODO:Can we avoid this copy?
                dbg_default_trace("key of image is: " + value.get_key_ref());
                dbg_default_trace("frame blob size is: " + std::to_string(value.blob.bytes_size()));
...
public:
    ...
    std::pair<std::string,double> infer(const ImageFrame& frame) {
        ...
        dbg_default_trace("the size of fd->data is: " + std::to_string(sizeof(fd->data)/sizeof(char)));
        dbg_default_trace("input shape expects size: " + std::to_string(input_shape.Size()));
        args_map["data"].SyncCopyFromCPU(reinterpret_cast<const mx_float*>(fd->data), input_shape.Size());
        dbg_default_trace("sync copy succedded");
...
#endif
        dbg_default_trace("idx inferred: " + std::to_string(idx));
        dbg_default_trace("species inferred: " + synset_vector[idx]);
...
#define DPL_CONF_REPORT_TO	"CASCADE/report_to"
    ...
    virtual void operator () (Action&& action, ICascadeContext* cascade_ctxt, uint32_t worker_id) {
        ...
        if (action.action_type == AT_FLOWER_NAME || action.action_type == AT_PET_BREED) {
            ...
            } else if (action.action_type == AT_PET_BREED) {
        ...
        } else if (action.action_type == AT_COW_RECOG) {
            ImageFrame* frame = dynamic_cast<ImageFrame*>(action.action_data.get());
            std::string name;
            double soft_max;

            std::tie(name,soft_max) = flower_ie.infer(*frame);
            std::vector<std::string> request = parse_request(frame->key);
            if (name.compare("azalea") != 0) {
                std::string grpc_frame_data = request_frame(request[1], std::stoi(request[2]));
                dbg_default_trace("size of frame from grpc server: " + std::to_string(grpc_frame_data.size()));
                PersistentCascadeStoreWithStringKey::ObjectType obj(request[0]+"/"+request[1]+"/"+request[2]+"/hi",reinterpret_cast<const char*>(&grpc_frame_data),sizeof(grpc_frame_data));
                auto result = ctxt->get_service_client_ref().template put<PersistentCascadeStoreWithStringKey>(obj);
                for (auto& reply_future:result.get()) {
                    auto reply = reply_future.second.get();
                    dbg_default_debug("node({}) replied with version:({:x},{}us)",reply_future.first,std::get<0>(reply),std::get<1>(reply));
